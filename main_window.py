...
class MainWindow(qtw.QWidget):
    def __init__(self):    
        ...
        self.meas_mode = 'IV'  # default measurement mode
    ...
    def read_current_out(self, voltage):
        ...
        time.sleep(
            (round(self.view.nplc_value.currentData() * 16.67) + float(self.view.source_delay_time_value.text()) + 10) / 1000)
    ...
    def measure_mode_changed(self):
        self.meas_mode = self.view.measure_mode_combo.currentText()
        # change the mode, the repeat value set back to 0

        self.clear_data()

        # print(self.meas_mode)
        self.view.message(self.meas_mode)
    ...
    def starter(self):
        ...
        self.visaResourceName = self.view.visa_name.currentText()
        ...
        try:
            ...
            if self.view.terminal_value.currentData() == 'FRON':
                self.SMU.set_front_terminal()
            elif self.view.terminal_value.currentData() == 'REAR':
    ...
    def iv_starter(self):
        ...
        # activeTerminal = Front/Rear (enum)
        self.startV = float(self.view.from_voltage_value.text())
        # print(self.from_voltage_value.text())
        # print(self.startV)
        self.stopV = float(self.view.to_voltage_value.text())
        # print(self.stopV)
        self.stepV = float(self.view.step_voltage_value.text())
        self.numberStep = round((self.stopV - self.startV) / self.stepV)
        ...
        self.SMU.set_source_voltage_delay_time(float(self.view.source_delay_time_value.text()))
        ...
        if self.view.voltage_range_combo.currentText() == 'Auto':
            ...
            self.SMU.set_voltage_range_value(self.view.voltage_range_combo.currentData())
        ...
        self.SMU.set_measure_current_range(self.view.current_range_combo.currentData())
        # current compliance SOUR:VOLT:ILIM 1
        self.SMU.set_measure_current_limit(self.view.current_range_combo.currentData())
        # nplc setting
        self.SMU.set_measure_current_nplc(self.view.nplc_value.currentData())
        ...
        file_name = 'IV' + self.view.file_name_text.text() + currentTime + '.txt'
        self.filepath = os.path.join(self.view.folder_location_text.text(), file_name)
        ...
        try:
            ...
            # start timer here
            time_out = round(self.view.nplc_value.currentData() * 16.67) + round(
                float(self.view.source_delay_time_value.text())) + 50
    ...
    def rt_starter(self):
        ...
        if self.view.rt_voltage_range_combo.currentText() == 'Auto':
            ...
            # select V-source range
            self.SMU.set_voltage_range_value(self.view.rt_voltage_range_combo.currentData())
        # select V source amplitude
        self.SMU.set_voltage_level(self.view.rt_voltage_set_value.text())
        # select measure function
        self.SMU.set_measure_mode_current()
        self.SMU.set_measure_current_limit(self.view.rt_current_range_combo.currentData())
        # current range = current compliance
        self.SMU.set_measure_current_range(self.view.rt_current_range_combo.currentData())
        # nplc setting
        self.SMU.set_measure_current_nplc(self.view.nplc_value.currentData())
        ...
        if self.repeat == 0:
            self.goto_voltage(float(self.view.rt_voltage_set_value.text()), 10)
        else:
            self.SMU.set_voltage_level(self.view.rt_voltage_set_value.text())
            time.sleep(0.05)
        ...
        currentTime = datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S')
        file_name = 'RT' + self.view.file_name_text.text() + currentTime + '.txt'
        self.filepath = os.path.join(self.view.folder_location_text.text(), file_name)
        ...
        try:
            ...
            self.timer_function(round(float(self.view.rt_aperture_value.text()) * 1000))
